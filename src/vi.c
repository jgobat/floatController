...
// cmds modifying text[]
static const char modifying_cmds[] = "aAcCdDiIJoOpPrRs""xX<>~";
#endif
...
#define old_offset refresh__old_offset
	...
	if (ENABLE_FEATURE_VI_WIN_RESIZE IF_FEATURE_VI_ASK_TERMINAL(&& !G.get_rowcol_error) ) {
		...
		if (c != columns || r != rows) {
            ...
            if (screen) new_screen(rows, columns);
		}
#endif
	}
...
//	p = q = save_dot = buf; // quiet the compiler
 ...
 key_cmd_mode:
	...
	case '.':			// .- repeat the last modifying command
		...
		if (lmc_len != 0) {
			ioq = ioq_start = strndup(last_modifying_cmd, lmc_len);
		}
		break;
#endif
#if ENABLE_FEATURE_VI_SEARCH
